...
*Setup instructions for a gitian build of Moneta using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Moneta
...
- [Building Moneta](#building-moneta)
...
Clone the git repositories for moneta and gitian and then checkout the moneta version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/moneta-project/moneta.git
cd moneta
...
Follow the instructions in [doc/release-process.md](release-process.md) in the moneta repository
...
To build Moneta (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the moneta repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/moneta/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/moneta-project/moneta
...
For example:
```bash
URL=https://github.com/user/moneta.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit moneta=${COMMIT} --url moneta=${URL} ../moneta/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit moneta=${COMMIT} --url moneta=${URL} ../moneta/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit moneta=${COMMIT} --url moneta=${URL} ../moneta/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/moneta-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/moneta-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/moneta-build.assert
```
...
[moneta/gitian.sigs](https://github.com/moneta-project/gitian.sigs.MONET/) repository, or if that's not possible create a pull
request. 
