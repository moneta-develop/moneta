...
static bool AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-help"))
    {
        ...
        std::string strUsage = _("Moneta Core moneta-tx utility version") + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  moneta-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded moneta transaction") + "\n" +
              "  moneta-tx [options] -create [commands]   " + _("Create hex-encoded moneta transaction") + "\n" +
              "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded moneta transaction
